...
-- Q25. Count delivered vs not delivered in one row
-- CASE returns 1 for matching rows and NULL otherwise, and COUNT ignores NULLs.
SELECT
...
--   succeeds but the credit fails, the whole thing rolls back so money is never
--   lost.
-- Consistency: a transaction moves the database from one valid state to another,
--   respecting all constraints. A transfer that would make a balance negative is
--   rejected.
-- Isolation:  concurrent transactions don't see each other's half-finished work.
--   Two people buying the last item in stock won't both succeed and oversell it.
-- Durability: once committed, the change survives a crash. An order placed just
--   before the server dies is still there after restart.

-- Q27. Place a new order, its items, and update stock - all atomically.
-- The procedure rolls everything back if any statement fails.
DELIMITER $$
...
-- Plain BEGIN...COMMIT version (PostgreSQL or generic SQL):
-- BEGIN;
--     INSERT INTO orders (order_id, customer_id, order_date, status, total_amount)
